    ...
    VkMemoryPropertyFlags bufferProperties, VkBuffer* vkBuffer, VkDeviceMemory* vkDeviceMemory)
{
    ...
    bufferInfo.usage              = bufferUsage;                   // Multiple types of buffer possible (example: Vertex Buffer)
    bufferInfo.sharingMode        = VK_SHARING_MODE_EXCLUSIVE;     // Similar to Swap Chain images, can share vertex buffers

    VkResult result = vkCreateBuffer(device, &bufferInfo, nullptr, vkBuffer);
    ...
    result = vkAllocateMemory(device, &memoryAllocInfo, nullptr, vkDeviceMemory);
    if (result != VK_SUCCESS)
    {
